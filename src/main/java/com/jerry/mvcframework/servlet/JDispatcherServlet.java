...
public class JDispatcherServlet extends HttpServlet {
    ...
    private Object doDispatch(HttpServletRequest request, HttpServletResponse response) throws IOException, InvocationTargetException, IllegalAccessException {
        ...
        for(int i = 0; i < parameterTypes.length;i++){
            ...
            if(clazz.isInstance(request)){
                parameterObject[i]  = request;
            }else if(clazz.isInstance(response)){
                parameterObject[i]  = response;
            }else{
                JRequestParam requestParam = parameters[i].getAnnotation(JRequestParam.class);
                if(requestParam != null){
                    parameterObject[i] = request.getParameter(requestParam.value().trim());
                }
            }
        }
        Object invoke = handler.getMethod().invoke(handler.getController(), parameterObject);
